<?php
namespace Base\Service\Sms;

use Base\App\Configuration\ParametersInterface;
use Base\Service\NotifierInterface;
...
class Smsc implements NotifierInterface
{
    protected $configuration = [];

    public function __construct(ParametersInterface $parametrs)
    {
        $this->configuration = $parametrs;
    }


    public function send(Message $message)
    {
        return $this->send_sms($message->to, $message->body);
    }
    ...
    {
        return mail("send@send.smsc.ru", "", $this->configuration->get('login') . ":" . $this->configuration->get('password') . ":$id:$time:$translit,$format,$sender:$phones:$message", "From: " . SMTP_FROM . "\nContent-Type: text/plain; charset=" . SMSC_CHARSET . "\n");
    }
    ...
    {
        $url = (SMSC_HTTPS ? "https" : "http") . "://smsc.ru/sys/$cmd.php?login=" . urlencode($this->configuration->get('login')) . "&psw=" . urlencode($this->configuration->get('password')) . "&fmt=1&charset=" . SMSC_CHARSET . "&" . $arg;
